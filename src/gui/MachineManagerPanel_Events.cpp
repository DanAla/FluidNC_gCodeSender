...
{
    // Auto-connect functionality temporarily disabled to resolve network handling issues
    LOG_INFO("Auto-connect functionality is currently disabled");
    return;
}
...
void MachineManagerPanel::OnAddMachine(wxCommandEvent& WXUNUSED(event))
{
    ...
    if (dialog.ShowModal() == wxID_OK) {
        ...
        // Auto-connect functionality temporarily disabled
        if (data.autoConnect) {
            LOG_INFO("Auto-connect enabled for new machine but functionality is currently disabled: " + data.name.ToStdString());
            NotificationSystem::Instance().ShowInfo(
                "Auto-connect Disabled",
                wxString::Format("Machine '%s' has been added. Auto-connect is currently disabled.", data.name)
            );
        }
...
void MachineManagerPanel::OnEditMachine(wxCommandEvent& WXUNUSED(event))
{
    ...
    if (dialog.ShowModal() == wxID_OK) {
        ...
        if (updatedData.autoConnect && !m_machines[selectedIndex].connected) {
            LOG_INFO("Auto-connect enabled for edited machine but functionality is currently disabled: " + updatedData.name.ToStdString());
            NotificationSystem::Instance().ShowInfo(
                "Auto-connect Disabled",
                wxString::Format("Machine '%s' has been updated. Auto-connect is currently disabled.", updatedData.name)
            );
        }
...
{
    // Create and show the network scan dialog
    std::unique_ptr<NetworkScanDialog> scanDialog(new NetworkScanDialog(this));
